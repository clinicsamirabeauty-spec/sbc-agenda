...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (error || !data) {
      ...
      return res.status(200).json({ status: 'trial', plan: 'trial', trial_end: trialEnd.toISOString(), days_left: 30 });
    }
    ...
    if (data.assinatura_status === 'trial' && data.trial_fim) {
      ...
      return res.status(200).json({ status: 'trial', plan: 'trial', trial_end: data.trial_fim, days_left: daysLeft });
    }
    ...
    if (data.assinatura_status === 'ativa') {
      ...
      return res.status(200).json({ status: 'ativa', plan: data.plano || 'mensal', period_end: data.assinatura_fim });
    }
